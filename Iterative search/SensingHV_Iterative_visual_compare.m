...
Q_true = deg2rad(3); % elevation of HV (answer)
...
% 정확도 계산 (100 - 오차 백분율)
Q_accuracy_percent = 100 - Q_error_percent;
w_accuracy_percent = 100 - w_error_percent;

% 정확도 그래프 생성
hold on; % 여러 플롯을 한 그래프에 그리기 위함

plot([0, iterations], Q_accuracy_percent, 'LineWidth', 2, 'Marker', 'o', 'DisplayName', 'Q Accuracy (%)');
plot([0, iterations], w_accuracy_percent, 'LineWidth', 2, 'Marker', 'x', 'DisplayName', 'w Accuracy (%)');

xlabel('Iteration','FontSize',15);
xticks(0:1:max(iterations)); % iteration이 0부터 시작한다고 가정
ylabel('Accuracy (%)','FontSize',15);
title('Accuracy of HV driving angle estimation over Iterations','FontSize',15);
